const OWM_KEY = CONFIG.OWM_API_KEY;
const BANDUNG_CENTER = [CONFIG.MAP_CENTER_LAT, CONFIG.MAP_CENTER_LNG];

const ALERT_DESC = {
  bahaya:
    "Risiko banjir sangat tinggi. Segera waspada penuh, hindari area bantaran dan dataran rendah.",
  waspada:
    "Kondisi rawan banjir. Pantau terus perkembangan cuaca dan siaga mengungsi jika diperlukan.",
...
function hitungRisiko(zoneLevel, rainMM, lat, lng) {
  ...
  const mitigasi =
    mitigasiPompa(lat, lng) + mitigasiKolam(lat, lng) + mitigasiSumur(lat, lng);
...
function zonaAt(latlng) {
  ...
  for (const z of [2, 1, 0]) {
    ...
    if (
      feat &&
      pointInMultiPolygon(latlng.lng, latlng.lat, feat.geometry.coordinates)
    )
      return z;
  }
  return 0;
...
["paneZona", "paneSungai", "paneBatas", "paneInfra", "paneTitik"].forEach(
  (name, i) => {
  ...
  .then((data) => {
    L.geoJSON(data, {
      ...
      style: (f) => {
        ...
        return {
          color: "#3b82f6",
          weight: r.includes("utama") || r.includes("induk") ? 2.5 : 1.5,
          opacity: 0.7,
        };
      },
    }).addTo(layers.sungai);
  ...
  .then((data) => {
    ...
    L.geoJSON(data, {
      ...
      onEachFeature: (f, l) => {
        ...
        l.on("click", (e) =>
          bukaPanel((p.Kelurahan || "—").toUpperCase(), e.latlng),
        );
      },
    }).addTo(layers.historis);
  });
  ...
  .then((data) => {
    L.geoJSON(data, {
      ...
      onEachFeature: (f, l) => {
        ...
        let centroid = null;
        try {
          centroid = l.getBounds().getCenter();
        } catch (_) {}
        ...
        l.on("mouseover", function (e) {
          ...
          fetch(
            `https://api.openweathermap.org/data/2.5/weather?lat=${e.latlng.lat}&lon=${e.latlng.lng}&appid=${OWM_KEY}&units=metric&lang=id`,
          )
            .then((r) => r.json())
            .then((w) => {
        ...
        l.on("mouseout", function () {
          this.setStyle({ weight: 0.8, color: "#94a3b8", dashArray: "3,3" });
        });
...
fetch("data/RumahPompa.json")
  ...
  .then((data) => {
    ...
    L.geoJSON(data, {
      pane: "paneInfra",
      pointToLayer: (f, latlng) =>
        L.marker(latlng, { icon: buatIkon("#0ea5e9", "P") }),
      onEachFeature: (f, l) => {
        const p = f.properties;
        l.bindTooltip(
          `
...
fetch("data/KolamRetensi.json")
  ...
  .then((data) => {
    ...
    L.geoJSON(data, {
      pane: "paneInfra",
      pointToLayer: (f, latlng) =>
        L.marker(latlng, { icon: buatIkon("#8b5cf6", "K") }),
      onEachFeature: (f, l) => {
        const p = f.properties;
        l.bindTooltip(
          `
...
fetch("data/SumurImbuhan.json")
  ...
  .then((data) => {
    ...
    L.geoJSON(data, {
      pane: "paneInfra",
      pointToLayer: (f, latlng) =>
        L.marker(latlng, { icon: buatIkon("#10b981", "S") }),
      onEachFeature: (f, l) => {
        const p = f.properties;
        l.bindTooltip(
          `
...
function bukaPanel(nama, latlng) {
  ...
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?lat=${latlng.lat}&lon=${latlng.lng}&appid=${OWM_KEY}&units=metric&lang=id`,
  )
    .then((r) => r.json())
    .then((w) => {
      const rain = w.rain ? w.rain["1h"] || w.rain["3h"] || 0 : 0;
...
document.querySelectorAll(".toggle").forEach((t) => {
  t.addEventListener("click", () => {
    ...
    isOn
      ? map.addLayer(layers[t.dataset.layer])
      : map.removeLayer(layers[t.dataset.layer]);
  });
});
